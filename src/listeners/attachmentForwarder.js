...
exports.registerAttachmentForwarder = registerAttachmentForwarder;
...
async function sendChunkSeries(msg, chunks, preferReply = true) {
    if (!chunks.length)
        return;
    try {
...
function createSharpPipeline(sharpMod, input) {
    try {
...
function extractAssistantText(ns) {
    try {
        let out = '';
        ...
        if (!out && typeof ns?.content === 'string')
            return ns.content;
        return out;
    }
    catch {
 ...
 */
function registerAttachmentForwarder(client) {
    ...
    client.on(discord_js_1.Events.MessageCreate, async (msg) => {
        ...
        if (urls.length === 0 && nonImageAttachments.length > 0) {
            ...
            const { sendMessage, MessageType } = await Promise.resolve().then(() => __importStar(require('../messages.js')));
            try {
                await msg.channel.sendTyping();
                await sendMessage(msg, MessageType.MENTION);
            }
            catch (err) {
        ...
        let longerTimeout = null; // For "taking longer" message
        let processingReply = null; // Store reply for editing
        try {
...
async function compressImage(buffer, mediaType, index, total, forceDimensionCheck = false) {
...
async function forwardImagesToLetta(urls, userId, userText, statusMessage = null) {
