 ...
 */
function validateAndSanitizeText(text: string, maxLength: number): string {
...
export class TTSService {
  ...
  async cleanupOldFiles(maxAgeMs: number = 3600000): Promise<number> {
    try {
      ...
      for (const file of files) {
        if (!file.startsWith('tts_')) continue; // Only clean our generated files
  ...
  async getHealth(): Promise<{
    healthy: boolean;
    initialized: boolean;
    modelsAvailable: { low: boolean; medium: boolean };
    audioDir: { exists: boolean; writable: boolean };
  }> {
    const health = {
      healthy: false,
